...
from smoe.data.llama_moefication_datasets import (
    CommonDataset,
    LineByLineJsonlTextDataset,
)
...
parser.add_argument(
    ...
    type=str,
    default="/home/dongdz/workspace/moefication/llama_data_cache/",
)
parser.add_argument(
    "--save_path",
    type=str,
...
args.save_path = os.path.join(
    args.save_path, os.path.split(args.model_path)[1] + "-Hidden-Features"
)
    ...
    for file_name in os.listdir(args.train_data_path):
        if key in file_name and file_name.endswith(".jsonl"):
            cached_file_path = os.path.join(
                args.train_data_cache_path, key + "_cached.pth"
            )
            if os.path.exists(cached_file_path):
                ...
                all_datasets[key] = LineByLineJsonlTextDataset(
                    tokenizer, file_path=raw_file_path, block_size=2048
                )
                ...
                with open(cached_file_path, "wb") as file:
                    torch.save(
                        all_datasets[key].examples,
                        os.path.join(args.train_data_cache_path, key + "_cached.pth"),
                    )
            print(
                "Dataset "
                + key
                ...
                + str(
                    sum(
                        [
                            torch.sum(all_datasets[key][i] != 2).item()
                            for i in range(len(all_datasets[key]))
                        ]
                    )
                )
                + " total tokens."
...
for (
    key
) in dataset_weight.keys():  # assume this key is 100% used, O(n^2) time complexity
    ...
    for (
        ...
        dataset_weight.keys()
    ):  # check if other key can provide enough number of batches
        if key == check_key:
            continue
        if int(
            len(all_datasets[key]) * (dataset_weight[check_key] / dataset_weight[key])
        ) > len(
            all_datasets[check_key]
        ):  # the check_key cannot provide enough data
            this_available_num = None  # not satisfied
            break
        else:  # this check_key is satisfied
            this_available_num[check_key] = int(
                len(all_datasets[key])
                * (dataset_weight[check_key] / dataset_weight[key])
            )
...
for (
    ...
    dataset_weight.keys()
):  # reset the number of examples by max_available_num and data_use_percent
    all_datasets[key].examples = random.sample(
        all_datasets[key].examples, int(max_available_num[key] * args.data_use_percent)
    )  # 按照batch分配，由于padding的存在，token的比例会有误差
...
for key in dataset_weight.keys():
    ...
    print(
        ...
        + str(
            sum(
                [
                    torch.sum(training_dataset[-1][i] != 2).item()
...
train_sampler = torch.utils.data.distributed.DistributedSampler(
    training_dataset, rank=args.local_rank, shuffle=True, drop_last=True
)
...
print(
    ...
    + str(
        sum(
            [
                torch.sum(training_dataset[i] != 2).item()
...
process_bar2 = tqdm(
    range(len(train_loader)), desc="forward step", position=0, leave=True
)
