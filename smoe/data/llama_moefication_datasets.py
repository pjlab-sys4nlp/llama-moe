            ...
            for line in lines:
                try:
                    ...
                    if chunk_num == 1:  # 只有一块，则直接对原文本编码
                        ...
                        for content_tokenized_block in self.split_list_by_n(
                            content_tokenized, block_size
                        ):  # chunk content by block_size
                            content_block = " ".join(
                                content_tokenized_block
                            )  # 重新组合为文本形式，以使用tokenizer的encode函数进行自动处理
                            content_encoding = tokenizer(
                                content_block,
                                add_special_tokens=True,
                                truncation=True,
...
class ShardDatasetForMoEGate(Dataset):  # 从多个数据shard文件中进行数据集读取
    def __init__(
        ...
        hidden_gate_outputs_filename_list = os.listdir(hidden_gate_outputs_path)
        hidden_gate_outputs_filename_list.sort()

        assert len(hidden_inputs_filename_list) == len(
            hidden_gate_outputs_filename_list
        )

        self.parallel_mode = parallel_mode
        assert self.parallel_mode in ("shards", "workers")

        if file_load_index_range is None:
            file_load_index_range = [0, len(hidden_inputs_filename_list) - 1]
        hidden_inputs_filename_list = hidden_inputs_filename_list[
            file_load_index_range[0] : file_load_index_range[1]
        ]
        hidden_gate_outputs_filename_list = hidden_gate_outputs_filename_list[
            file_load_index_range[0] : file_load_index_range[1]
        ]

        if (
            self.parallel_mode == "shards"
        ):  # 提前读取shards_in_memory个shard文件到内存并合并，之后各个workers并行读取内存中的数据
            hidden_inputs_filepath_list = [
                os.path.join(hidden_inputs_path, name)
                for name in hidden_inputs_filename_list
            ]
            hidden_gate_outputs_filepath_list = [
                os.path.join(hidden_gate_outputs_path, name)
                for name in hidden_gate_outputs_filename_list
            ]

            self.chunked_hidden_inputs_filepath_list = []
            self.chunked_hidden_gate_outputs_filepath_list = []

            while len(hidden_inputs_filepath_list) > 0:
                self.chunked_hidden_inputs_filepath_list.append(
                    hidden_inputs_filepath_list[:shards_in_memory]
                )
                self.chunked_hidden_gate_outputs_filepath_list.append(
                    hidden_gate_outputs_filepath_list[:shards_in_memory]
                )
                hidden_inputs_filepath_list = hidden_inputs_filepath_list[
                    shards_in_memory:
                ]
                hidden_gate_outputs_filepath_list = hidden_gate_outputs_filepath_list[
                    shards_in_memory:
                ]
            ...
            self.hidden_inputs_filepath_list = [
                os.path.join(hidden_inputs_path, name)
                for name in hidden_inputs_filename_list
            ]
            self.hidden_gate_outputs_filepath_list = [
                os.path.join(hidden_gate_outputs_path, name)
                for name in hidden_gate_outputs_filename_list
            ]
    ...
    def __getitem__(self, i):
        if self.parallel_mode == "shards":
            return self.hidden_inputs_examples[i], self.hidden_gate_outputs_examples[i]

        elif self.parallel_mode == "workers":
            hidden_inputs = torch.load(
                self.hidden_inputs_filepath_list[i], map_location="cpu"
            )
            hidden_gate_outputs = torch.load(
                self.hidden_gate_outputs_filepath_list[i], map_location="cpu"
            )
            # print("*", hidden_inputs.shape)
            # print("*", hidden_gate_outputs.shape)
            return hidden_inputs, hidden_gate_outputs
    ...
    def load_shards(self):  # "shards"并行模式下使用
        ...
        if self.load_pos != object_load_pos:
            ...
            self.hidden_gate_outputs_examples = []

            for filepath in tqdm(
                self.chunked_hidden_inputs_filepath_list[self.load_pos],
                desc="loading hidden_inputs shards",
                leave=False,
            ...
            for filepath in tqdm(
                self.chunked_hidden_gate_outputs_filepath_list[self.load_pos],
                desc="loading hidden_gate_outputs shards",
                leave=False,
            ):  # 单进程读取，使用多进程会由于大量的内存交换而降低速度
                tensor = torch.load(filepath, map_location="cpu")
                tensor_list = torch.split(
                ...
                self.hidden_gate_outputs_examples.extend(tensor_list)
