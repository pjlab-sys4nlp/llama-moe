...
gpus=1
OMP_NUM_THREADS=8 srun --partition=MoE --job-name=convert --mpi=pmi2 --gres=gpu:${gpus} -n1 --ntasks-per-node=1 -c 24 --kill-on-bad-exit=1 \  python ${root_path}/run_moefication/llama_convert.py \
