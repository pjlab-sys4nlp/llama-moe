...
gpus=0
cpus=16
for retain_percent in 99 98 95 90 80 60 40 25 20; do
  ...
  #  split_file_path=${data_path}/moefication_results/prune/Random/${expert_index}-0.${retain_percent}Percent-${expert_size}Neurons
  #  save_path=${data_path}/models/${convert_type}-Prune2/Random/${llama_size}-${expert_index}-0.${retain_percent}Percent-${expert_size}Neurons

  OMP_NUM_THREADS=8 srun --partition=MoE --job-name=prune-convert --mpi=pmi2 --gres=gpu:${gpus} -n1 --ntasks-per-node=1 -c ${cpus} --kill-on-bad-exit=1 \
